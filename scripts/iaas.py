...
commands.getstatusoutput("sudo chown apache iaasuser.py")
fil=open("/webcontent/scripts/iaasuser.py",'w')
fil.write("#!/usr/bin/python2\nimport commands\ncommands.getstatusoutput(\"sudo vncviewer {}:{}\")".format(serverip,port))     
fil.close()

clientStatus=commands.getstatusoutput("sshpass -p {0} scp -o stricthostkeychecking=no iaasuser.py root@{1}:/root/Desktop".format(clientpass,clientip))
